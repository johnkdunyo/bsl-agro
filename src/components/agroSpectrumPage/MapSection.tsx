...
const SingleRegionComponent = ({ name, id }: { name: string; id: number }) => {
  return (
    <div className="border flex items-center justify-center px-2 py-1.5 rounded-md shadow-xl relative">
      <button className="font-medium text-sm">{name}</button>
...
const MapSection = () => {
  return (
    <section className="bg-white py-20  h-full" id="where-we-are">
      {/* mobile */}
      <div className="flex flex-col gap-8 text-black custom-container text-base md:hidden">
        <h1 className="text-5xl font-semibold text-[#037B20] text-center md:text-left">
          Where We are Now
        </h1>
        <div>
          <p>
            In collection of data from the famers, Agrospectrum have covered the
            5 northern regions which includes:
          </p>
        </div>
        ...
        <div className="-mt-16">
          <p className="text-base leading-[2.2rem] text-left font-light">
            “The next step is to move to the southern part of the country to
            enumerate all farmers unto the platform to improve efficiency in the
            input of the subsidy programme by eliminating the manual system’’.
          </p>
        </div>
      </div>
      {/* desktop */}
      <div className="flex-col md:flex-row justify-between custom-container gap-10 md:gap-0 hidden md:flex">
        {/* map */}
        <div className="w-full relative  h-[40rem] md:h-[50rem]">
          {/* <SVGMap map={Nigeria} /> */}
